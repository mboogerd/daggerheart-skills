...
12. For each roster slot, decide whether it should be:
    ...
    Start the final answer with the exact title `# Combat Encounter Plan Template`. Do not add a different top heading.
...
14. Keep escalation compatible with the roster plan. Do not casually add unpaid reinforcements or mix multiple roles into one escalation note. If extra enemies appear because of a leader feature or similar exception, say that clearly.
